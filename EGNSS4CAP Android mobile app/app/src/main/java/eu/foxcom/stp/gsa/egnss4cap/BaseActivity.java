...
import eu.foxcom.stp.gsa.egnss4cap.MainService.BROADCAST_MSG;
import eu.foxcom.stp.gsa.egnss4cap.MainService.BROADCAST_REFRESH_TASKS_PARAMS;
...
public abstract class BaseActivity extends AppCompatActivity implements ServiceInit {
    ...
    public void serviceInit() {
        ...
        setKeepScreenOnActive(MS.isKeepScreenOnActive());
    ...
    protected void setKeepScreenOnActive(Intent intent) {
        if (!intent.hasExtra(MainService.BROADCAST_KEEP_SCREEN_ON_PARAMS.IS_ACTIVE.ID)) {
            return;
        }
        setKeepScreenOnActive(intent.getBooleanExtra(MainService.BROADCAST_KEEP_SCREEN_ON_PARAMS.IS_ACTIVE.ID, false));
    }

    protected void setKeepScreenOnActive(boolean isActive) {
        if (isActive) {
