...
import eu.foxcom.gnss_compare_core.FileLoggers.KalmanFilterFileLogger;


public class PedestrianStaticExtendedKalmanFilter extends eu.foxcom.gnss_compare_core.PvtMethods.PvtMethod {
...
//        SimpleMatrix sigma2C1 = new SimpleMatrix(CONSTELLATION_SIZE, 1);
        ...
        if(usedInCalculations > 0) {
            ...
            Coordinates coordinates = Coordinates.globalXYZInstance(
                    x_meas.get(idxX),
                    x_meas.get(idxY),
                    x_meas.get(idxZ));
            coordinates.setRefTime(new Time(DateTime.now().toDate().getTime()));
            return coordinates;
    ...
    public static void registerClass(){
        register(NAME, eu.foxcom.gnss_compare_core.PvtMethods.PedestrianStaticExtendedKalmanFilter.class);
    }
